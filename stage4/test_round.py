...
class RoundPlay(unittest.TestCase):
 ...
 def test_two_team_privacy_reveal_revision_and_shared_commit(self):
  ...
  try:
   ...
   self.shot('play-two-team-commit');self.click('Six months later');self.begin()
   for p in pages:
   ...
   e.locator('#game-mode').select_option('expedition');e.wait_for_url('**/expedition.html#*');e.locator('#loading').wait_for(state='hidden')
   expect(e.locator('#game-mode option[value=play]')).to_be_disabled();e.locator('#game-mode').select_option('negligence');e.wait_for_url('**/round.html#*');e.locator('#loading').wait_for(state='hidden')
   self.assertEqual(e.evaluate('roundDiagnostics().state.mission'),'negligence');self.assertEqual(e.evaluate('roundDiagnostics().state.phase'),'committed')
  finally:
 ...
 def test_phone_without_webgl_still_explores_and_plays(self):
  ...
  try:
   ...
   self.click('Six months later');self.begin();self.page.wait_for_function('!roundDiagnostics().busy');self.inspect('B');self.seek('recovery',10);self.click('Without care');self.shot('neglect-phone-no-webgl')
 ...
 def test_negligence_forecast_before_proposal_and_structure_tracks_year(self):
  ...
  self.click('Six months later');self.begin();self.page.wait_for_function('!roundDiagnostics().busy')
  ...
  self.inspect('C');self.seek('recovery',10);self.click('With care');self.shot('neglect-02-cared-close')
  self.assertEqual(self.page.evaluate('roundDiagnostics().forecast.alive'),80)
  self.click('Without care');self.shot('neglect-03-uncared-close');self.assertEqual(self.page.evaluate('roundDiagnostics().forecast.alive'),15)
  ...
  self.page.locator('#structure-lab').get_by_role('button',name='With care',exact=True).click();self.seek('structure-year',10);self.shot('neglect-06-structure-cared')
  ...
  self.page.locator('#structure-lab').get_by_role('button',name='Back',exact=True).click();self.inspect('D');self.click('Forest');self.page.wait_for_function('!roundDiagnostics().busy');self.shot('neglect-07-new-clearing')
