...
class RoundPlay(unittest.TestCase):
 ...
 def test_phone_without_webgl_still_explores_and_plays(self):
  ...
  try:
   ...
   self.page.locator('#role').select_option('ecology');self.begin();self.page.locator('[data-specimen="urochloa_decumbens"]').click();self.page.get_by_role('tab',name='Germination').click();self.shot('record-phone-no-webgl');expect(self.page.locator('.seed-clue')).to_contain_text('Patch D');self.click('Back')
  finally:b.close()
 ...
 def test_seed_study_and_broad_scar_playthrough(self):
  ...
  grass=Image.open(BytesIO(self.page.locator('#record-seed canvas').screenshot()))
  ...
  native=Image.open(BytesIO(self.page.locator('#record-seed canvas').screenshot()));self.assertGreater(sum(ImageStat.Stat(ImageChops.difference(grass,native)).mean),.5)
  ...
  self.inspect('D');self.page.locator('[data-specimen="urochloa_decumbens"]').click();self.page.get_by_role('tab',name='Dispersal').click();expect(self.page.locator('.seed-clue')).to_contain_text('Patch D (C4):');self.shot('record-new-patch');self.click('Back')
