...
export function navigation(mode,role){
 ...
 nav.innerHTML='<select id="game-mode" aria-label="Game mode"><option value="expedition">Expedition</option><option value="play">Play</option></select><select id="role" aria-label="Team view"><option value="removal">Removal</option><option value="ecology">Ecologist</option><option value="room">Room</option></select><button id="teams-open">Teams</button>';
...
export function expeditionNavigation(){
 ...
 const dialog=document.createElement('dialog');dialog.id='expedition-teams';dialog.innerHTML='<div class="dialog-heading"><h1>Teams</h1><button>Back</button></div><p>Share one link with each team. Start in Expedition, then choose Play.</p><div class="team-links"></div>';
 ...
 nav.querySelector('#teams-open').onclick=async()=>{try{const s=await session(),links=dialog.querySelector('.team-links');links.replaceChildren();
  ...
  else dialog.querySelector('p').textContent='Your team explores here, then chooses Play. The room makes the shared decision.';
  dialog.showModal();
