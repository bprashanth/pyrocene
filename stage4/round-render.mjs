...
const effectGLSL=`uniform float recovery,removeSector,restoreSector,nativeLoss;
...
export class RoundForest extends ExpeditionForest{
 constructor(host,options){
  super(host,options);this.plan=null;this.recovery=0;this.roundFX={recovery:{value:0},removeSector:{value:-1},restoreSector:{value:-1},nativeLoss:{value:0}};
  this.roundLabels=document.createElement('div');this.roundLabels.className='round-labels';this.overlay.append(this.roundLabels);
  this.patchPins=PATCHES.map(p=>{const b=document.createElement('button');b.textContent=p.key;b.setAttribute('aria-label','Inspect patch '+p.key);b.onclick=()=>options.select(p.id);this.roundLabels.append(b);return {p,b};});
 ...
 makeGrowth(){
  ...
  const m=new T.ShaderMaterial({transparent:true,depthWrite:false,uniforms:{recovery:this.roundFX.recovery,fire:{value:this.arrivalTexture},hasFire:{value:this.fire?1:0},fireTime:{value:this.fireTime*240}},
   vertexShader:`uniform float recovery;varying vec2 mapUv;varying float alpha;void main(){vec3 p=position;mapUv=vec2((p.x+450.)/900.,(p.z+450.)/900.);p.y*=recovery;alpha=recovery*.8;vec4 mv=modelViewMatrix*vec4(p,1.);gl_Position=projectionMatrix*mv;gl_PointSize=clamp(800./-mv.z,1.5,3.);}`,
 ...
 setFire(arrival,duration){
  ...
  for(const c of [this.detailCloud,this.growthCloud])if(c?.material.uniforms.hasFire)c.material.uniforms.hasFire.value=arrival?1:0;
  this.cpuKey=null;
 }
 setFireTime(fraction){
  super.setFireTime(fraction);if(this.emberTime)this.emberTime.value=fraction*240;
  for(const c of [this.detailCloud,this.growthCloud])if(c?.material.uniforms.fireTime)c.material.uniforms.fireTime.value=fraction*240;
  this.cpuKey=null;
 }
 ...
 drawFallback(){
  ...
  for(const p of PATCHES){ctx.strokeStyle=p.id===this.selected?'#edce89':'#507c67';ctx.beginPath();const cx=(p.id%6)*150-375,cz=Math.floor(p.id/6)*150-375;[[-74,-74],[74,-74],[74,74],[-74,74],[-74,-74]].forEach(([x,z],i)=>{v.set(cx+x,2,cz+z).project(this.camera);const a=(v.x*.5+.5)*w,b=(-v.y*.5+.5)*h;i?ctx.lineTo(a,b):ctx.moveTo(a,b);});ctx.stroke();}
 }
}
