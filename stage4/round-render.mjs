...
const effectGLSL=`uniform float recovery,removeSector,restoreSector,nativeLoss,successionActive,nativeFraction,invasiveCover,extraSector;
...
export class RoundForest extends ExpeditionForest{
 constructor(host,options){
  super(host,options);this.plan=null;this.recovery=0;this.roundFX={recovery:{value:0},removeSector:{value:-1},restoreSector:{value:-1},nativeLoss:{value:0},successionActive:{value:0},nativeFraction:{value:1},heightScale:{value:1},invasiveCover:{value:0},extraSector:{value:-1}};
 ...
 setSuccession(forecast=null,extraRemoval=null){
  this.succession=forecast;const f=this.roundFX;f.successionActive.value=forecast?1:0;f.nativeFraction.value=forecast?.nativeFraction??1;f.heightScale.value=forecast?.heightScale??1;f.invasiveCover.value=forecast?.invasive??0;f.extraSector.value=extraRemoval?patch(extraRemoval).id:-1;
 ...
 makeGrowth(){
  ...
  this.growthCloud=new T.Points(g,m);this.growthCloud.frustumCulled=false;this.scene.add(this.growthCloud);
 }
 makeRegrowth(cx,cz){
  if(this.regrowthCloud){this.scene.remove(this.regrowthCloud);this.regrowthCloud.geometry.dispose();this.regrowthCloud.material.dispose();this.regrowthCloud=null;}
  this.regrowthPositions=new Float32Array(this.regrowthSource||[]);for(let n=0;n<this.regrowthPositions.length;n+=3){this.regrowthPositions[n]+=cx;this.regrowthPositions[n+2]+=cz;}
  if(this.fallback)return;
  const g=new T.BufferGeometry();g.setAttribute('position',new T.BufferAttribute(this.regrowthPositions,3));
  const m=new T.ShaderMaterial({transparent:true,depthWrite:false,uniforms:{...this.roundFX,fire:{value:this.arrivalTexture},hasFire:{value:this.fire?1:0},fireTime:{value:this.fireTime*240}},
  ...
  this.regrowthCloud=new T.Points(g,m);this.regrowthCloud.visible=!!this.succession;this.regrowthCloud.frustumCulled=false;this.scene.add(this.regrowthCloud);
 }
 makeEmbers(){
  if(this.fallback)return;
  const positions=[],arrivals=[],seeds=[];
  for(let row=0;row<60;row++)for(let col=0;col<60;col++)for(let k=0;k<3;k++){positions.push(col*15-450+3+k*4,1,row*15-450+4+(k*7)%9);arrivals.push(-1);seeds.push((row*13+col*7+k*11)%29/29);}
  ...
  const m=new T.ShaderMaterial({transparent:true,depthWrite:false,blending:T.AdditiveBlending,uniforms:{time:this.emberTime},
   vertexShader:`attribute float arrival,seed;uniform float time;varying float a;varying float hot;void main(){float age=time-arrival; hot=1.-clamp(age/16.,0.,1.);a=arrival>=0.&&age>=0.?.4+hot*.6:0.;vec3 p=position;p.y+=hot*fract(age*.2+seed)*(4.+seed*9.);vec4 mv=modelViewMatrix*vec4(p,1.);gl_Position=projectionMatrix*mv;gl_PointSize=clamp(2400./-mv.z,2.,5.);}`,
   fragmentShader:`varying float a;varying float hot;void main(){float d=length(gl_PointCoord-.5);if(d>.5||a<=0.)discard;vec3 c=mix(vec3(.47,.27,.14),vec3(1.,.62,.16),hot);gl_FragColor=vec4(c,a*(1.-d*2.));}`});
 ...
 setFire(arrival,duration){
  ...
  if(this.embers){this.embers.visible=!!arrival;const a=this.embers.geometry.attributes.arrival;for(let i=0;i<a.count;i++){const v=arrival?.[Math.floor(i/3)];a.array[i]=Number.isFinite(v)?v/duration*240:-1;}a.needsUpdate=true;}
  for(const c of [this.detailCloud,this.growthCloud,this.regrowthCloud])if(c?.material.uniforms.hasFire)c.material.uniforms.hasFire.value=arrival?1:0;
  this.cpuKey=null;
 }
 setFireTime(fraction){
  super.setFireTime(fraction);if(this.emberTime)this.emberTime.value=fraction*240;
  for(const c of [this.detailCloud,this.growthCloud,this.regrowthCloud])if(c?.material.uniforms.fireTime)c.material.uniforms.fireTime.value=fraction*240;
  this.cpuKey=null;
 }
 ...
 drawFallback(){
  ...
  const draw=(x,y,z,wood=false,growth=false,weeds=false)=>{
   ...
   if(weeds){y*=.6+this.succession.invasive;const v=Math.sin(Math.floor(x/8)*12.9898+Math.floor(z/8)*78.233)*43758.5453,group=v-Math.floor(v);alpha=this.succession.invasive*.65*smooth(group-.08,group+.08,this.succession.invasive);colour='#e8488c';}
   if(!growth&&id===this.roundFX.extraSector.value&&y<3)alpha*=.18;
