...
test('all eighteen taxa are reachable and all four missions can be completed without hidden prerequisites',()=>{
 ...
 assert.equal(knownPlants(s).length,18);assert.equal(new Set(TAXA).size,18);assert(progress(s).every(p=>p.done));
...
test('every plant has a sourced photograph and catalogue record',()=>{
 ...
 for(const id of TAXA){const sp=c.species.find(s=>s.id===id);assert(sp?.sources.length,id);assert(photos[sp.photoAssetId],id);}
 assert.equal(c.species.filter(s=>s.status==='Native').length,15);assert.equal(c.species.filter(s=>s.status==='Invasive').length,3);
});
