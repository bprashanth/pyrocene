...
class SharedRoundRules(unittest.TestCase):
 ...
 def test_negligence_carries_commit_and_requires_agreement(self):
  ...
  with self.assertRaises(RoundError):self.action('advance',role='ecology')
  self.action('advance');self.assertEqual(self.s['mission'],'negligence');self.assertEqual(self.s['previous'],old)
  ...
  with self.assertRaises(RoundError):self.action('advance')
  self.action('replay');self.assertEqual(self.s['mission'],'cooperation');self.assertIsNone(self.s['previous'])
