...
"$PYTHON_BIN" "$SCRIPT_DIR/render_spectral_seamless.py" \
  --amazon "$AMAZON_LIDAR" \
  --neon-artifact "$NEON_ARTIFACT" \
  --amazon-spectral-artifact "$AMAZON_SPECTRAL" \
  --captions "$SCRIPT_DIR/spectral_seamless_captions.json" \
  --output "$OUTPUT_DIR/amazon-spectral-seamless-v1.mp4"

ffmpeg -hide_banner -loglevel error -y \
  -i "$OUTPUT_DIR/amazon-spectral-seamless-v1.mp4" -an \
  -vf scale=1920:1080:flags=lanczos \
  -c:v libx264 -preset slow -crf 20 -maxrate 10M -bufsize 20M \
  -pix_fmt yuv420p -movflags +faststart \
  "$REVIEW_DIR/amazon-spectral-seamless-v1-1080p.mp4"
